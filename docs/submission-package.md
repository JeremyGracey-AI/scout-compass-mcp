...
4. **(1:45)** Human asks for the audit (`run_audit` is human-triggered — the agent never audits itself unprompted; say this on camera). **The audit shows SIX findings across four kinds — pan over the report:** uncited decision, two stale skills, two `stale-skill-cleared` lines naming the decision that cited each of those skills, and a low-confidence repeat. Call out the cleared lines: staleness is derived from the append-only `compass/citations.jsonl`, and a finding the agent's own citation clears changes kind and names the decision instead of silently disappearing. The drafted proposal's "What the agent missed" section cites `kn-payment-policy` — derived from real vault content, not invented.
